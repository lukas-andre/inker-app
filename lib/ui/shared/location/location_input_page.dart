...
class _LocationInputPageState extends State<LocationInputPage> {
  ...
  Future<void> _fetchPredictions() async {
    ...
    try {
      ...
      final predictions =
          await placesService.getAutoComplete(_addressController.text);
  ...
  void _validateForm() {
    setState(() {
      _isFormValid = _nameController.text.isNotEmpty && _selectedPlace != null;
    });
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_selectedPlace != null && !_isLoadingPlace)
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<AddressType>(
              ...
              items: AddressType.values
                  .map<DropdownMenuItem<AddressType>>((AddressType value) {
                return DropdownMenuItem<AddressType>(
                  value: value,
                  child: Text(
                    value.toString().split(
                        '.')[1], // Convert enum to string (e.g., "STUDIO")
  ...
  Widget _buildPredictionsList() {
    ...
    if (_predictions.length == 1 &&
        _predictions.first.description == '__NO_RESULTS__') {
      return Container(
        margin: const EdgeInsets.only(top: 8),
        padding: const EdgeInsets.symmetric(vertical: 16, horizontal: 16),
        decoration: BoxDecoration(
            ...
            Row(
              children: [
                Icon(Icons.location_on,
                    color: Theme.of(context).colorScheme.secondary, size: 16.0),
